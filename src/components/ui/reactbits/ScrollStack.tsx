...
function StackCard({
  ...
  const translateY = useTransform(progress, [0, 1], [index * 24, index * -12]);
  const opacity = useTransform(progress, [0, 0.3 + index * 0.1], [0.3, 1]);
  ...
  return (
    <div ref={containerRef} className={clsx('relative min-h-[320px]', className)}>
      {items.map((item, index) => (
        <StackCard key={item.title} item={item} index={index} progress={scrollYProgress} />
      ))}
    </div>
  );
}
